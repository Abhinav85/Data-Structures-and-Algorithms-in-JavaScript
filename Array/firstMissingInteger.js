...
// Answer - We can approach the problem in the following fashion.
// 1 - Why are they saking for constant space, This might be a pointer that the array itself is
// The storage facility. But why would that be so?

// Maybe because of the fact that the smallest integer has to be either between 1-N(Array length)
// or be N (1-N because we have to find the SMALLEST integer.) and (N of 1-N are already present in the Array)

// So then, All you need to do is this. The elements that are equal to or smaller than the length of the array
// You need to make their indexes i.e A[element] not A[i] at ehich element is found negative. To be exact A[element -1].
// And then, the first positive number is your answer.

// The problem you'll face - Edge case, what about [1], surely this will return 0, but for this you need to return i=1 too


firstMissingInt = A => {
    var i = 0;

    // Make the non positive elements grater than the length of the Array
    for(i = 0; i < A.length; i++) {
        if(A[i] <= 0) A[i] = A.length+1;
    }

    // For the positive elements that are smaller than or equal to the length of the array, make their indexes negative
    for(i = 0; i < A.length; i++){
